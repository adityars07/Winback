...
def execute_action(txn: Transaction, final_action: str) -> dict:
    ...
    if final_action == "retry_payment":
        ...
        if success:
            ...
            outcome["message"] = f"Retried payment for INR {txn.amount:,.2f} - SUCCESS"
        else:
            outcome["message"] = f"Retried payment for INR {txn.amount:,.2f} - FAILED (will retry in next batch)"
        ...
        if success:
            ...
            txn.recovered_amount = txn.amount
            outcome["success"] = True
            outcome["message"] = f"Sent payment link to {txn.customer_id} for INR {txn.amount:,.2f} - RECOVERED"
        else:
            outcome["message"] = f"Sent payment link to {txn.customer_id} for INR {txn.amount:,.2f} - awaiting response"
        ...
        if success:
            ...
            txn.recovered_amount = txn.amount
            outcome["success"] = True
            outcome["message"] = f"Sent WhatsApp reminder to {txn.customer_id} for INR {txn.amount:,.2f} - RECOVERED"
        else:
            outcome["message"] = f"Sent WhatsApp reminder to {txn.customer_id} for INR {txn.amount:,.2f} - awaiting response"

    elif final_action == "escalate_to_human":
        txn.status = "escalated"
        outcome["message"] = f"Escalated {txn.txn_id} (INR {txn.amount:,.2f}) to human agent"

    elif final_action == "mark_unrecoverable":
        txn.status = "unrecoverable"
        outcome["message"] = f"Marked {txn.txn_id} (INR {txn.amount:,.2f}) as unrecoverable"

    else:
        outcome["message"] = f"Unknown action '{final_action}' for {txn.txn_id}"

    txn.final_action_taken = final_action

    # Log to console safely for Windows terminal
